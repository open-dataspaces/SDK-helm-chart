...
| charts/payment       | 精算・決済サービスのデプロイ定義ファイル格納先 |
...
2. charts/payment/values.yaml 中の以下のパラメータに, chart/l3/values.yaml 中の `l3KeycloakIntrospectClientSecret` と同じ値を設定し、精算・決済サービスを再起動して反映します。
